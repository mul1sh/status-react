  ...
  (letsubs [blocked-contacts-count [:contacts/blocked-count]
    ...
    [react/view {:flex 1
     ...
     [react/scroll-view {:style {:background-color colors/white
                                 :padding-vertical 8}}
      [list.views/big-list-item
       {:text                (i18n/label :t/blocked-contacts)
        :icon                :main-icons/cancel
        :icon-color          colors/red
        :accessibility-label :blocked-contacts-list-button
        :accessory-value     blocked-contacts-count
        :action-fn           #(re-frame/dispatch [:navigate-to :blocked-contacts-list])}]
      [react/view {:height 16}]
      [list.views/big-list-item
       {:text                (i18n/label :t/add-contact)
        :text-color          colors/blue
        :icon                :main-icons/add
        :hide-chevron?       false
        :accessibility-label :add-contact-button
